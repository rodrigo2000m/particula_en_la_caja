...
function Shrodinger_solution(n1, n2, n3, l1, l2, l3) { // se puede usar una sola funcion, cuando es 1d n2=n3=0 y l2=l3=1 (para evitar problemas con las cuentas)
    ...
    var new_y = [];
    var new_z = [];
    var completed_wavefunction = [];
    var structured_wavefunction = []
    x.forEach(i => {
        ...
        y.forEach(j => {
            ...
            //new_x.push(i)
            //new_y.push(j)
            completed_wavefunction.push(Math.sqrt(2 / (l1)) * Math.sin(n1 * Math.PI * i / l1) * Math.sqrt(2 / (l2)) * Math.sin(n2 * Math.PI * j / l2))
        })
        structured_wavefunction.push(completed_wavefunction)
        console.log(completed_wavefunction)
        completed_wavefunction = []
    });
    ...
    return { x, y, z, new_x, new_y, new_z, E, wavefunction, structured_wavefunction, completed_wavefunction, wavefunction_x, wavefunction_y, wavefunction_z, prob_density } //E, x, y, z wavefunction
}
